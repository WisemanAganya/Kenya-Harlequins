          ...
          <div className="flex justify-between items-center h-20">
            {/* Logo Area */}
             <NavLink to="/" className="flex items-center gap-3 group shrink-0">
               <div className="w-12 h-12 md:w-14 md:h-14 rounded-full overflow-hidden bg-white border border-slate-100 shadow-sm flex items-center justify-center p-1 group-hover:border-quins-magenta transition-colors duration-300">
                 <img src="/assets/kh/KH Logo.PNG" alt="Logo" className="w-full h-full object-contain" />
               </div>
               <div className="flex flex-col">
                 <span className="font-black text-sm md:text-base uppercase tracking-tighter leading-none text-slate-900 group-hover:text-quins-magenta transition">Kenya Harlequins</span>
                 <span className="text-[10px] font-bold text-quins-magenta uppercase tracking-widest leading-none">#SSS</span>
               </div>
            </NavLink>
            ...
            <div className="hidden lg:flex items-center space-x-0.5">
              {NAV_ITEMS.map((item) => (
                <NavLink
                  ...
                  to={item.path}
                  className={({ isActive }) =>
                    `px-3 py-2 rounded-lg text-[11px] font-black uppercase tracking-tight transition-all duration-300 ${
                      isActive
                        ? 'text-quins-magenta bg-slate-50'
                        : 'text-slate-500 hover:text-quins-blue hover:bg-slate-50'
                    }`
                  }
                >
                  {item.label}
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="relative p-2 text-slate-400 hover:text-quins-magenta transition-transform hover:scale-110 group"
                >
                  <ShoppingCart size={20} />
                  {totalItems > 0 && (
                    <span className="absolute -top-1 -right-1 bg-quins-magenta text-white text-[9px] font-black rounded-full w-4 h-4 flex items-center justify-center shadow-lg">
                      {totalItems}
                    </span>
                  )}
                </button>
                ...
                {session ? (
                  <NavLink to="/dashboard" className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-quins-magenta transition-all shadow-md group">
                    <User size={14} className="text-quins-magenta group-hover:text-white transition" />
                    Portal
                  </NavLink>
                ) : (
                  <NavLink to="/auth" className="px-4 py-2 bg-quins-blue text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-sky-600 transition-all shadow-md">
                    Login
                  </NavLink>
                )}
                
                <NavLink to="/membership" className="px-4 py-2 border-2 border-slate-200 text-slate-900 rounded-xl text-[10px] font-black uppercase tracking-widest hover:border-quins-magenta hover:text-quins-magenta transition-all">
                  Join Us
                </NavLink>
              </div>
            </div>
